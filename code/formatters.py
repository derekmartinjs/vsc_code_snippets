...
# last_phrase has the last phrase, WITHOUT formatting.
# This is needed for reformatting
last_phrase = ""

# formatted_phrase_history keeps the most recent formatted phrase, WITH formatting 
...
def format_phrase(m: Union[str, Phrase], fmtrs: str):
  ...
  if isinstance(m, str):
    ...
    words = actions.dictate.replace_words(m)
...
class ImmuneString(object):
  """ Wrapper that takes a string immune from formatting """
  def __init__(self, string):
    self.string = string

@mod.capture(
  # Add anything else into this that you want to be able to speak during a formatter

  rule="(<self.symbol_key> | numb <number>)"
)
...
@mod.action_class
class Actions:
  def formatted_text(phrase: Union[str, Phrase], formatters: str):
